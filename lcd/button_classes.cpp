...
PWMDigOutRGB::PWMDigOutRGB(PinName t_pin1, PinName t_pin2, PinName t_pin3)
...
void PWMDigOutRGB::set_color(uint8_t t_r, uint8_t t_g, uint8_t t_b) {
	m_val1 = t_r;
	m_val2 = t_g;
	m_val3 = t_b;
...
uint8_t PWMDigOutRGB::get_r() const { return m_val1; }
uint8_t PWMDigOutRGB::get_g() const { return m_val2; }
uint8_t PWMDigOutRGB::get_b() const { return m_val3; }

void PWMDigOut::pwm_control() {
	if(this->m_ticks < this->m_T) {
		this->m_led = 1;
	} else {
		this->m_led = 0;
	}
...
uint8_t PWMDigOut::get_bright() const { return m_jas; }
...
int BTN_NOT::get_btn_state() { return m_btn.read(); }
BTN_SET::BTN_SET(PinName t_pin, bool* t_true_state, bool* t_false_state)
	: m_btn(t_pin)
